...
Return ONLY the enhanced request, nothing else. Keep it concise (1-2 sentences).`,
    ...
    Apply the requested changes while maintaining the Tailwind CSS styling approach.`,
        ...
        await prisma.conversation.create({
            data: {
                role: "assistant",
                content: "I'hv made change to your website! You can now preview it",
                projectId,
            },
        });
...
export const rollbackToVersion = async (req, res) => {
    try {
        ...
        await prisma.conversation.create({
            data: {
                role: "assistant",
                content: "I've rolled back your website to selected versiton. You can now preview it",
                projectId,
            },
        });
        res.json({ message: "Version rooled back" });
    }
    catch (error) {
        console.log(error.code || error.message);
        res.status(500).json({ message: error.message });
...
// Controller Function to delet a project
...
export const getProjectById = async (req, res) => {
    try {
        ...
        if (!project || project.isPublished === false || !project?.current_code) {
            return res.status(404).json({ message: "The project is not available." });
        }
