...
const Home = () => {
  ...
  const onSubmitHandler = async (e: React.FormEvent) => {
    ...
    try {
      if (!session?.user) {
        return toast.error("Please sing in to create a project.");
      } else if (!input.trim()) {
  ...
  return (
    <div>
      <section className="flex flex-col items-center text-white text-sm pb-20 px-4 font-poppins">
        ...
        <h1 className="text-center text-[40px] leading-12 md:text-6xl md:leading-[70px] mt-4 font-semibold max-w-3xl">
          Turn thoughts into Website instantly, with AI.
        </h1>

        <p className="text-center text-base max-w-md mt-2">
          Create, customize and publish website faster than ever with iour AI.
        </p>
