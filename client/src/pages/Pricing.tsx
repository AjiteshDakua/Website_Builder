...
const Pricing = () => {
  ...
  const handlePurchase = async (planId: string) => {
    try {
      if (!session?.user)
        return toast("Please log in to continue with your purchase.");
    ...
    <>
      <div className="w-full max-w-5xl mx-auto z-20 max-md:px-4 min-h-[80vh]">
        ...
        <p className="mx-auto text-center text-sm max-w-md mt-10 text-white/60 font-light">
          Each project creation or revision uses{" "}
          <span className="text-white">5 credits</span>. You can purchase
          additional credits anytime to continue creating new projects.
        </p>
      </div>
      <Footer />
    </>
